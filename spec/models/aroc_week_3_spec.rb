...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = Order.joins(user: :order_items).select(:name).where('item_id = ?', @item_8.id).distinct.pluck(:name)
    # ------------------------------------------------------------
  ...
  it '17. returns the name of items associated with a specific order' do
    ...
    names = Order.joins(:items).where('orders.id = ?', order_id).pluck(:name)
    # ------------------------------------------------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------------------------------------------------
    # ------------------ Using ActiveRecord ----------------------
    order_id = Order.where('user_id = ?', @user_3.id).order(:id).third.id
    items_for_user_3_third_order = Item.joins(:orders).where('user_id = ? and order_id = ?', @user_3.id, order_id).pluck(:name)
