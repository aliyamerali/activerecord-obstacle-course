...
# ----------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # NOTES:
    # # .count AR method returns a hash value
    # # in this we needed to use a select statement with a SQL COUNT command
    ## instead to return in the format we needed.
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User
          .select('users.name AS user_name, orders.id AS order_id, FLOOR(orders.amount / COUNT(order_items.id)) AS avg_item_cost')
          .joins(:order_items)
